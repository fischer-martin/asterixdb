...
class computeMBR implements Summary<Rectangle> {
    ...
    public void add(Rectangle k) {
        if(k.x1 < MBR.x1) MBR.x1 = k.x1;
        if(k.x2 > MBR.x2) MBR.x2 = k.x2;
        if(k.y1 < MBR.y1) MBR.y1 = k.y1;
        if(k.y2 > MBR.y2) MBR.y2 = k.y2;
    }

    @Override
    public void add(Summary<Rectangle> s) {
        computeMBR c = (computeMBR) s;
        if(c.MBR.x1 < MBR.x1) MBR.x1 = c.MBR.x1;
        if(c.MBR.x2 > MBR.x2) MBR.x2 = c.MBR.x2;
        if(c.MBR.y1 < MBR.y1) MBR.y1 = c.MBR.y1;
        if(c.MBR.y2 > MBR.y2) MBR.y2 = c.MBR.y2;
    }
}
...
public class SpatialJoin implements FlexibleJoin<Rectangle, SpatialJoinConfiguration>{
    ...
    public SpatialJoinConfiguration divide(Summary<Rectangle> s1, Summary<Rectangle> s2) {
        ...
        if(c1.MBR.x1 < c2.MBR.x1) c1.MBR.x1 = c2.MBR.x1;
        if(c1.MBR.x2 > c2.MBR.x2) c1.MBR.x2 = c2.MBR.x2;
        if(c1.MBR.y1 < c2.MBR.y1) c1.MBR.y1 = c2.MBR.y1;
        if(c1.MBR.y2 > c2.MBR.y2) c1.MBR.y2 = c2.MBR.y2;
